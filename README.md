...
read it by. `[` and `]` scroll it, as does the wheel, and it says where in the
file it is. Brackets rather than shifted arrows, because whether a terminal
reports shift with an arrow at all is up to the terminal — plenty send the bare
sequence, and then the keystroke meant for the preview moves the list instead.
Shift and an arrow work too, where they arrive. `p` puts the panel away when
the list wants the width, and on a narrow terminal it moves below the list.
